...
SYSTEM_PROMPTS = {
    "shortlisting": "You are skilled at shortlisting. Select the top 5 best vendors from the provided list based on the criteria mentioned in the user query. For example, if they care about eco-friendliness, look through the media information and make sure it does not have any bad climate related press. If they have a hard budget, make sure the vendors do not cross that. When you reject a vendor, make sure you have an explanation. The return format should be the shortlisted vendors with their information, and then all the rejected vendors with reasons for rejection.",
